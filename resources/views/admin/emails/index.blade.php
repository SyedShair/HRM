...
<style>
    ...
    .compose-employee-list label:last-child {
        border-bottom: none;
    }
    ...
    .ec-toast-success .ec-toast-bar {
        background: var(--ec-green);
    }

    .ec-toast-error .ec-toast-bar {
        background: #dc2626;
    }

    .ec-toast-warning .ec-toast-bar {
        background: #d97706;
    }

    .ec-toast-info .ec-toast-bar {
        background: #2563eb;
    }
    ...
    .ec-loader-overlay.show .ec-loader-box {
        transform: scale(1);
    }
    ...
    @media (max-width: 767px) {
        ...
        table.email-table td.text-right {
            text-align: left;
        }
    ...
    <div class="ec-header">
        <div>
            ...
            <p class="page-subtitle">
                {{ __('Send passport, visa, share code, and general HR emails to employees') }}
            </p>
        </div>
                    ...
                    <table class="email-table">
                        ...
                        <tbody>
                            ...
                            @forelse($passportList as $person)
                                ...
                                @php
                                    $lsPassport = isset($lastSent[$person->id])
                                        ? $lastSent[$person->id]->firstWhere('type', 'passport_expiry')
                                        : null;
                                ...
                                <tr>
                                    ...
                                    <td data-label="{{ __('Employee') }}">
                                        ...
                                        <div class="emp-cell">
                                            ...
                                            <span class="emp-avatar-fallback">
                                                {{ mb_substr($person->firstname, 0, 1) }}{{ mb_substr($person->lastname, 0, 1) }}
                                            </span>
                                            ...
                                            <div>
                                                ...
                                                <div class="emp-company {{ $person->company ? '' : 'no-company' }}">
                                                    {{ $person->company ?? __('No company assigned') }}
                                                </div>
                                    ...
                                    <td data-label="{{ __('Passport No') }}">
                                        {{ $person->nationalid ?: '—' }}
                                    </td>

                                    <td data-label="{{ __('Expiry Date') }}">
                                    ...
                                    <td data-label="{{ __('Actions') }}" class="text-right">
                                        ...
                                        @if($person->emailaddress)
                                            ...
                                            <form
                                                action="{{ route('emails.passport', $person->id) }}"
                                                method="post"
                                                class="send-reminder-form"
                                            ...
                                            <button
                                                type="button"
                                                class="ui button small basic btn-send send-confirm-trigger"
                                            ...
                                            <div
                                                class="last-sent-hint"
                                                id="{{ $hintId }}"
                                                style="{{ $lsPassport ? '' : 'display:none;' }}"
                                            >
                                                <i class="ui check circle outline icon"></i>
                                                {{ __('Last sent') }}:

                                                <span class="hint-time">
                                                    {{ $lsPassport
                                                        ? \Carbon\Carbon::parse($lsPassport->last_sent_at)->diffForHumans()
                                                        : ''
                                                    }}
                                                </span>
                                            </div>
                    ...
                    <table class="email-table">
                        ...
                        <tbody>
                            ...
                            @forelse($visaList as $person)
                                ...
                                @php
                                    $lsVisa = isset($lastSent[$person->id])
                                        ? $lastSent[$person->id]->firstWhere('type', 'visa_expiry')
                                        : null;
                                ...
                                <tr>
                                    ...
                                    <td data-label="{{ __('Actions') }}" class="text-right">
                                        ...
                                        @if($person->emailaddress)
                                            ...
                                            <form
                                                action="{{ route('emails.visa', $person->id) }}"
                                                method="post"
                                                class="send-reminder-form"
                                                style="display:inline;"
                                            >
                                            ...
                                            >
                                                ...
                                                <span class="hint-time">
                                                    {{ $lsVisa
                                                        ? \Carbon\Carbon::parse($lsVisa->last_sent_at)->diffForHumans()
                                                        : ''
                                                    }}
                                                </span>
                                            </div>
                    ...
                    <table class="email-table">
                        ...
                        <tbody>
                            ...
                            @forelse($shareCodeList as $person)
                                ...
                                @php
                                    $lsShare = isset($lastSent[$person->id])
                                        ? $lastSent[$person->id]->firstWhere('type', 'sharecode_expiry')
                                        : null;
                                ...
                                <tr>
                                    ...
                                    <td data-label="{{ __('Actions') }}" class="text-right">
                                        ...
                                        @if($person->emailaddress)
                                            ...
                                            <form
                                                action="{{ route('emails.sharecode', $person->id) }}"
                                                method="post"
                                                class="send-reminder-form"
                                                style="display:inline;"
                                            >
                                            ...
                                            >
                                                ...
                                                <span class="hint-time">
                                                    {{ $lsShare
                                                        ? \Carbon\Carbon::parse($lsShare->last_sent_at)->diffForHumans()
                                                        : ''
                                                    }}
                                                </span>
                                            </div>
                ...
                >
                    ...
                    <div class="compose-grid">
                        ...
                        <div>
                            ...
                            <label class="compose-side-label">
                                {{ __('Recipients') }}
                            </label>
                            ...
                            <div class="select-all-row">
                                ...
                                <span class="cell-muted">
                                    {{ __('Select one or more employees') }}
                                </span>

                                <span>
                                    <a
                                        href="#"
                                        onclick="toggleAllRecipients(true); return false;"
                                    >
                                        {{ __('Select all') }}
                                    </a>
                                    ...
                                    <a
                                        href="#"
                                        onclick="toggleAllRecipients(false); return false;"
                                    >
                                        {{ __('Clear') }}
                                    </a>
                                </span>
                        ...
                        <div>
                            ...
                            <div class="field">
                                ...
                                <input
                                    ...
                                    name="subject"
                                    value="{{ old('subject') }}"
                                    placeholder="{{ __('e.g. Important Update from HR') }}"
                                >
                            ...
                            <div class="field" style="margin-top:16px;">
                                ...
                                <button
                                    type="submit"
                                    class="ui green button small btn-send"
                                >
                                    <i class="ui send icon"></i>
                                    {{ __('Send Email') }}
                                </button>
...
<div class="ui small modal" id="send-confirm-modal">
    ...
    <div class="content">
        ...
        <div class="confirm-message">
            ...
            <strong id="send-confirm-doctype"></strong>

            {{ __('expiry reminder to') }}

            <strong id="send-confirm-employee"></strong>
    ...
    <div class="actions">
        ...
        <button
            type="button"
            class="ui button cancel-btn"
            onclick="$('#send-confirm-modal').modal('hide')"
        >
            {{ __('Cancel') }}
        </button>
...
<div
    id="ec-toast-container"
    class="ec-toast-container"
    aria-live="polite"
></div>

<div
    id="ec-loader-overlay"
    class="ec-loader-overlay"
>
    ...
    <div class="ec-loader-box">
        ...
        <div
            class="ec-loader-subtext"
            id="ec-loader-subtext"
        ></div>
...
function toggleAllRecipients(checked) {
    ...
    document
        .querySelectorAll('.recipient-checkbox:not(:disabled)')
        .forEach(function (cb) {
...
function showToast(type, message, timeout = 5500) {
    ...
    toast.innerHTML =
        ...
        '<div class="ec-toast-close" onclick="dismissToast(\'' + id + '\')">' +
...
function showLoader(text, subtext) {
    ...
    document.getElementById('ec-loader-text').textContent =
        text || '{{ __('Please wait...') }}';

    document.getElementById('ec-loader-subtext').textContent =
        subtext || '';
...
function hideLoader() {
    ...
    document
        .getElementById('ec-loader-overlay')
...
function openSendConfirm(button) {
    ...
    /*
     * The trigger button sits right after its matching hidden form
     * in the DOM, so this targets the correct employee form.
     */

    pendingSendForm = button.previousElementSibling;
    pendingSendButton = button;

    document.getElementById('send-confirm-employee').textContent =
        button.dataset.employee;

    document.getElementById('send-confirm-doctype').textContent =
...
document
    .getElementById('send-confirm-ok-btn')
    .addEventListener('click', async function () {

        if (!pendingSendForm || !pendingSendButton) {
            return;
        }
...
function markSentButton(button) {
    ...
    if (hintId) {
        ...
        if (hint) {
            ...
            if (timeEl) {
                timeEl.textContent = '{{ __('just now') }}';
            }
    ...
    button.innerHTML =
        '<i class="ui check icon"></i> {{ __('Sent') }}';
...
if (composeForm) {
    ...
    composeForm.addEventListener('submit', async function (e) {
        ...
        const checked =
            composeForm.querySelectorAll(
                '.recipient-checkbox:checked'
            );

        const subjectField =
            composeForm.querySelector('[name="subject"]');

        const messageField =
            composeForm.querySelector('[name="message"]');

        composeForm
            .querySelectorAll('.field.error-field')
        ...
        if (checked.length === 0) {
            ...
            showToast(
                'warning',
                '{{ __('Please select at least one recipient.') }}'
            );
        ...
        const submitBtn =
            composeForm.querySelector('button[type="submit"]');
